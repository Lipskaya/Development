...
public class CertificateDaoImpl implements CertificateDao {
  ...
  private static final Logger LOGGER = Logger
      .getLogger(CertificateDaoImpl.class.getCanonicalName());
  private static final String SQL_TOTAL_CERTIFICATES = "SELECT count(*) FROM gift_certificate";
  private static final String SQL_GET_CERTIFICATE_BY_ID = "SELECT id,name,description,price,create_day,last_update_date FROM gift_certificate WHERE id = ?";
  private static final String SQL_UPDATE_CERTIFICATE = "UPDATE gift_certificate SET name = ?,description = ?, price = ?, last_update_date = ? WHERE ID = ?";
  private static final String SQL_DELETE_CERTIFICATE = "DELETE FROM gift_certificate WHERE id = ?";
  private static final String CERTIFICATE_NOT_FOUND_START_MESSAGE = "Certificate with ID = ";
  ...
  /**
   * Retrieves the certificate from database by provided id Tag list will be empty. Use TagDao to
  ...
  public Certificate getById(Integer id) {
    ...
    try {
      ...
      LOGGER.error(CERTIFICATE_NOT_FOUND_START_MESSAGE, e);
  ...
  /**
   * Retrieves list of all Certificate from database.
   *
  ...
  public List<Certificate> getAll(Sort sortRequest) {
    LOGGER.debug(LOG_GET_ALL_START + sortRequest);
  ...
  public Certificate insert(Certificate certificate) {
    ...
    LOGGER.debug(LOG_INSERT_END + certificate);
    return certificate;
  }
  ...
  public Certificate update(Certificate newCertificate) {
    ...
    int updatedCertificateRows = jdbcTemplate
        .update(SQL_UPDATE_CERTIFICATE, name, description,
            price, newCertificate.getLastUpdateDate().toString(),
            newCertificate.getId());
    if (updatedCertificateRows == 0) {
      LOGGER.error(CERTIFICATE_NOT_FOUND_START_MESSAGE);
      throw new NotFoundException(
          CERTIFICATE_NOT_FOUND_START_MESSAGE + newCertificate.getId() + NOT_FOUND_END_MESSAGE,
          ErrorCode.ERROR_CERTIFICATE);
    }
    LOGGER.debug(LOG_UPDATE_END + newCertificate);
  ...
  public void delete(Certificate certificate) {
    ...
    LOGGER.debug(LOG_DELETE_END + certificate);
  }
